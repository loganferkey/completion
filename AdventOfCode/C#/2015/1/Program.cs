...
int FindFirstBasement(string array) {
    int floor = 0;
    for (int i = 0; i < array.Length; i++) {
        floor += array[i] == '(' ? 1 : -1;
        if (floor == -1) {
            floor = i + 1; // for some reason they index at 1 instead of 0 (booooooooooooo)
            break; // assign floor to index and break returning first character that took him to the basement
        }
    }
    return floor;
}
...
// Test cases
Console.WriteLine(FindFloor(puzzleInput));
Console.WriteLine(FindFirstBasement(puzzleInput));
// woohoo!
